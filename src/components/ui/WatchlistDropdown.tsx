...
export function WatchlistDropdown({ animeId, episodeId }: WatchlistDropdownProps) {
  ...
  const handleStatusSelect = async (status: WatchStatus) => {
    ...
    try {
      ...
      const numericAnimeId = typeof animeId === 'string' ? parseInt(animeId) : animeId;
      const numericEpisodeId = episodeId ? (typeof episodeId === 'string' ? parseInt(episodeId) : episodeId) : null;
      ...
      setFeedback({ type: 'success', message: `Added to ${status.replace('_', ' ')}!` });
    } catch (error) {
  ...
  return (
    <>
      {/* Feedback Message */}
      {feedback && (
        <div className={`absolute bottom-full left-0 mb-2 px-3 py-2 rounded-lg text-sm font-medium text-white whitespace-nowrap z-50 ${
