...
const mockActivity = [
  ...
  {
    type: "comment",
    text: "New comment on 'Attack on Titan'",
    time: "15m ago",
  },
...
const mockIssues = [
  {
    id: 1,
    type: "critical",
    text: "Video playback error on 'Jujutsu Kaisen'",
    date: "2024-07-16",
  },
  {
    id: 2,
    type: "moderate",
    text: "Subtitle sync issue on 'Demon Slayer'",
    date: "2024-07-15",
  },
];
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        <div className="bg-gray-900 rounded-xl p-5 flex flex-col items-center shadow relative">
          ...
          <div className="text-2xl font-bold text-white">
            {mockStats.users.count.toLocaleString()}
          </div>
        ...
        <div className="bg-gray-900 rounded-xl p-5 flex flex-col items-center shadow relative">
          ...
          <div className="text-2xl font-bold text-white">
            {mockStats.requests.count}
          </div>
          <div className="text-gray-400 text-xs mb-1">Pending Requests</div>
          <div
            className={`flex items-center gap-1 text-xs font-semibold ${
              mockStats.requests.growth >= 0 ? "text-green-400" : "text-red-400"
            }`}>
            {mockStats.requests.growth >= 0 ? (
              <ArrowUpRight className="w-4 h-4" />
            ) : (
              <ArrowDownRight className="w-4 h-4" />
            )}{" "}
            {Math.abs(mockStats.requests.growth)}%
          </div>
        </div>
        <div className="bg-gray-900 rounded-xl p-5 flex flex-col items-center shadow relative">
          <AlertTriangle className="w-7 h-7 text-red-400 mb-2" />
          <div className="text-2xl font-bold text-white">
            {mockStats.issues.count}
          </div>
          <div className="text-gray-400 text-xs mb-1">Reported Issues</div>
          <div className="flex items-center gap-2 text-xs">
            <span className="text-red-400 font-semibold">
              Critical: {mockStats.issues.critical}
            </span>
            <span className="text-yellow-400 font-semibold">
              Moderate: {mockStats.issues.moderate}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-gray-900 rounded-xl p-6 shadow flex flex-col">
          ...
          <ul className="flex-1 flex flex-col gap-3">
            {(showAllActivity ? mockActivity : mockActivity.slice(0, 4)).map(
              (item, i) => (
                <li
                  key={i}
        ...
        <div className="bg-gray-900 rounded-xl p-6 shadow flex flex-col gap-6">
          ...
          <div>
            <div className="font-bold text-lg text-white mb-2 flex items-center gap-2">
              <MessageCircle className="w-5 h-5 text-blue-400" /> Active
              Discussions
            </div>
            <ul className="flex flex-col gap-2">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {mockSearchTerms.map((term, i) => (
                <span
                  key={i}
                  className="bg-gray-800 text-green-300 px-3 py-1 rounded-full text-xs font-semibold">
                  {term}
                </span>
              ))}
            </div>
        ...
        <div className="flex flex-col gap-6">
            ...
            <ul className="flex-1 flex flex-col gap-3">
              ...
              {mockRequests.map((req) => (
                <li
                  ...
                  <span className="bg-yellow-700 text-yellow-200 px-2 py-1 rounded text-xs font-bold ml-2">
                    {req.status}
                  </span>
                </li>
              ))}
          ...
          <div className="bg-gray-900 rounded-xl p-6 shadow flex flex-col">
            ...
            <ul className="flex-1 flex flex-col gap-3">
              {mockIssues.length === 0 && (
                <li className="text-gray-400 text-sm">No reported issues.</li>
              )}
