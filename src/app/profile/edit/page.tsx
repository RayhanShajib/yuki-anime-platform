      ...
      <main className="pt-16">
        <div className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-1 bg-gray-800 rounded-lg p-1 mb-6 sm:mb-8">
            {[
              { key: 'profile', label: 'Profile Info', shortLabel: 'Profile', icon: User },
              { key: 'privacy', label: 'Privacy', shortLabel: 'Privacy', icon: Eye },
              { key: 'notifications', label: 'Notifications', shortLabel: 'Notifs', icon: AlertCircle },
            ].map(tab => {
              const Icon = tab.icon;
              return (
                <button
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleInputChange('profileVisibility', e.target.value, 'preferences')}
                      className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                    {[
                      ...
                      <label key={setting.key} className="flex items-center space-x-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={userData.preferences[setting.key as keyof typeof userData.preferences]}
                          onChange={(e) => handleInputChange(setting.key, e.target.checked, 'preferences')}
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-8">
              ...
              <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                ...
                <div className="space-y-4">
                  {[
                    { key: 'emailUpdates', label: 'Weekly Digest', description: 'Get a weekly summary of new anime and episodes' },
                    { key: 'newEpisodes', label: 'New Episodes', description: 'Notify when new episodes of your watching list are available' },
                    { key: 'recommendations', label: 'Recommendations', description: 'Get personalized anime recommendations' },
                    { key: 'friendRequests', label: 'Friend Requests', description: 'Notify when someone sends you a friend request' },
                    { key: 'comments', label: 'Comments', description: 'Notify when someone comments on your reviews or posts' },
                    { key: 'reviews', label: 'Review Updates', description: 'Notify about reviews on anime in your list' },
                  ].map(setting => (
                    <div key={setting.key} className="flex items-start space-x-3">
                      <input
                        type="checkbox"
                        checked={userData.notifications[setting.key as keyof typeof userData.notifications]}
                        onChange={(e) => handleInputChange(setting.key, e.target.checked, 'notifications')}
