...
const userData = {
  ...
  nextLevelExp: 10000,
  badges: [
    { id: 1, name: 'Binge Watcher', icon: '🍿', description: 'Watched 10 episodes in one day' },
    { id: 2, name: 'Genre Explorer', icon: '🗺️', description: 'Watched anime from 15+ genres' },
    { id: 3, name: 'Early Bird', icon: '🌅', description: 'Watched 50+ ongoing series' },
    { id: 4, name: 'Completionist', icon: '✅', description: 'Completed 100+ anime series' },
  ],
  stats: {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex flex-col items-center sm:items-start md:flex-row md:items-center space-y-4 sm:space-y-6 md:space-y-0 md:space-x-8">
              ...
              <div className="relative">
                <img
                  ...
                  className="w-20 h-20 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-full border-4 border-purple-500"
                />
              ...
              <div className="flex-1 text-center sm:text-left">
                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0 mb-4">
                  <h1 className="text-2xl sm:text-3xl font-bold text-white">{userData.username}</h1>
                  <Link
                    href="/profile/edit"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
                  <div className="text-center bg-gray-800/30 rounded-lg p-3 sm:p-4">
                    <div className="text-xl sm:text-2xl font-bold text-purple-400">{userData.totalWatched}</div>
                    <div className="text-gray-400 text-xs sm:text-sm">Anime Watched</div>
                  </div>
                  <div className="text-center bg-gray-800/30 rounded-lg p-3 sm:p-4">
                    <div className="text-xl sm:text-2xl font-bold text-blue-400">{userData.totalHours.toLocaleString()}</div>
                    <div className="text-gray-400 text-xs sm:text-sm">Hours Watched</div>
                  </div>
                  <div className="text-center bg-gray-800/30 rounded-lg p-3 sm:p-4">
                    <div className="text-xl sm:text-2xl font-bold text-green-400">{userData.stats.averageRating}</div>
                    <div className="text-gray-400 text-xs sm:text-sm">Avg Rating</div>
                  </div>
                  <div className="text-center bg-gray-800/30 rounded-lg p-3 sm:p-4">
                    <div className="text-xl sm:text-2xl font-bold text-yellow-400">{userData.badges.length}</div>
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-2">
                    ...
                    <span className="text-gray-400">{userData.exp} / {userData.nextLevelExp} XP</span>
                  </div>
          ...
          <div className="bg-gray-800 rounded-lg p-1 mb-6 sm:mb-8">
            <div className="flex space-x-1 overflow-x-auto scrollbar-hide">
              {[
                { key: 'overview', label: 'Overview', shortLabel: 'Overview', icon: User },
                { key: 'watching', label: 'Currently Watching', shortLabel: 'Watching', icon: PlayCircle },
                { key: 'completed', label: 'Completed', shortLabel: 'Completed', icon: Trophy },
                { key: 'favorites', label: 'Favorites', shortLabel: 'Favorites', icon: Heart },
                { key: 'watchlist', label: 'Watchlist', shortLabel: 'Watchlist', icon: Bookmark },
                { key: 'stats', label: 'Statistics', shortLabel: 'Stats', icon: Activity },
              ].map(tab => {
                const Icon = tab.icon;
                return (
                  <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {userData.badges.map(badge => (
                    ...
                    >
                      ...
                      <p className="text-gray-400 text-xs sm:text-sm flex-1">{badge.description}</p>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div key={anime.id} className="relative">
                    ...
                    <div className="mt-2 bg-gray-800/50 rounded p-2">
                      <div className="text-xs sm:text-sm text-gray-300 mb-1">
                        Episode {Math.floor(Math.random() * 20) + 1} / {anime.episodes || '?'}
                      </div>
          ...
          {activeTab === 'stats' && (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                <div className="bg-gray-800/50 rounded-lg p-4 sm:p-6 border border-gray-700">
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                      <div className="text-xl sm:text-2xl font-bold text-white">{userData.stats.episodesWatched.toLocaleString()}</div>
                  ...
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-white">
                        {Math.floor(userData.stats.minutesWatched / 60).toLocaleString()}
                      </div>
                ...
                <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-white">{userData.stats.averageRating}</div>
                ...
                <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-white">{userData.stats.completedSeries}</div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Watching Status */}
                <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                  <h3 className="text-xl font-bold text-white mb-4">Watching Status</h3>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300">Completed</span>
                      <span className="text-green-400 font-semibold">{userData.stats.completedSeries}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300">Currently Watching</span>
                      <span className="text-blue-400 font-semibold">{userLists.recentlyWatched.length}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300">Plan to Watch</span>
                      <span className="text-purple-400 font-semibold">{userData.stats.planToWatch}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300">On Hold</span>
                      <span className="text-yellow-400 font-semibold">{userData.stats.onHoldSeries}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-300">Dropped</span>
                      <span className="text-red-400 font-semibold">{userData.stats.droppedSeries}</span>
                    </div>
                  </div>
                </div>
